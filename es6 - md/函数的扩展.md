...
如果出入undefined，将处罚该参数等于默认值，null则没有这个效果。
```js
...
add(...numbers) // 42
```
上面代码中，array.push(...items)和add(...numbers)这两行，都是函数调用，它们的参数都使用了扩展运算符。该运算符将一个数组，变为参数序列。
...
除了this，以下三个变量在箭头函数之中也是不存在的，指向外层函数的对应变量：arguemnts、super、new.target。
```js
function foo(){
    setTimeout(() => {
