...
fucntion nonenumerable(target, name, descriptor){
    descriptor.enumerable = false;
    return descriptor;
}
```
下面是@log修饰器，可以起到输出日志的作用。
```js
class Math {
  @log
  add(a, b) {
...
function foo(){}
```
上面的代码，意图是执行后counter等于1，但是实际上结果就是counter等于0。因为函数提升，使得实际执行的代码是下面这样的。
```js
@add
function foo() {
}
...
```js
import {readonly} from 'core-descorators';
...
deprecate或的deprecated修饰器在控制台显式一条警告，表示该方法将废除。
```js
...
```
（5）@suppresWarning
suppressWarning修饰器抑制decorated修饰器导致的console.warn()调用。但是，异步代码发出的调用除外。
```js
...
```
上面代码定义了一个名为publish的修饰器，它通过改写descriptor.value，使得原生方法被调用时，会自动发出一个事件。它使用的事件“发布/订阅”库是Postal.js。
...
```
以后只要调用someMethod或者anotherMethod，就会自动发出一个事件。
```js
...
接着，目标类再去继承这个混入类，就打到了“混入”foo方法的目的。
```js
...
首先，安装babel-core和babel-plugin-transform-decorators。由于后者包括在babel-preset-stage-0之中，所以改为安装babel-perset-stage-0亦可。
```shell
