...
```
注意，简写写法的属性名总是字符串，这会导致一些看上去比较奇怪的结果。
```js
var obj = {
    class (){}
};
...
obj.foo.name
// TypeError: Cannot read property 'name' of undfined
...
```
如果对象的方法是一个Symbol值，那么name属性返回的就是这个Symbol值的描述。
```js
...
---
##4.Object.is()
ES5比较两个值是否相等，只有两个运算符：相等运算符（==）和严格相等运算符（===）。它们都有缺点，前者会自动转换数据类型，后者的NaN不等于自身，以及+0等于-0。JavaScript缺乏一种预算，在所有环境中，只要两个值是一眼的，它们就应该相等。
...
有一些函数库提供Object.assign的定制版本（比如Lodash的_.defaultDeep方法），可以解决浅拷贝的问题，得到深拷贝的合并。
...
```
上面代码中，DEFAULTS对象是默认值，options对象是用户提供的参数。Object.assign方法将DEFAULTS合并成一个新对象，如果两者有同名属性，则option的属性值会覆盖DEFAULTS的属性值。
...
ES5有三个操作会忽略enumerable为false的属性。
 ...
 - Object.keys()：返回对象自身的所以可枚举的属性的键名
...
ES6一共有5种方法可以遍历对象的属性。
（1）for...in
for...in循环变量对象自身的和继承的可枚举属性（不含Symbol属性）。
（2）Object.keys(obj)
...
obj.method = function(){...};
```
该属性没有写入ES6的正文，而是写入了附录，原因是\__proto__前后的双下划线，说明它本质上是一个内部属性，而不是一个正式的对外的API，只是由于浏览器广泛支持，才被加入了ES6。标准明确规定，只有浏览器必须部署这个属性，其他运行环境不一定需要部署，而且新的代码最好认为这个属性是不存在的。因此，无论从语义的角度，还是从兼容性的角度，都不要使用这个属性，而是使用下面的Object.setPrototypeOf()（写操作）、Object.getPrototypeOf()（读操作）、Object.create()（生成操作）代替。
...
```
如果一个对象本身部署了\__proto__属性，则该属性的值就是对象的原型。
```js
...
Object.setPrototypeOf方法的作用与\__proto__相同，用来设置一个对象的prototype对象，返回参数对象本身。它是ES6正式推荐的设置原型对象的方法。
```js
// 格式
...
// ["foo", "baz"]
```
ES2017引入了跟Object.keys配套的Object.values和Object.entries，作为简历一个对象的补充手段，供for...of循环使用。
```js
...
---
###Object.values()
Object.values方法返回一个数组，成员是参数对象自身的（不含继承的）所有可比案例（enumerable）属性的键值。
```js
var obj = {foo: 'bar', baz: 42};
Object.values(obj)
// ["bar", 42]
...
Object.values(obj) // []
```
上面代码中，Object.create方法的第二个参数添加的对象属性（属性p），如果不显式声明，默认是不可遍历的，因为p的属性描述对象enumerable默认是false，Object.values不会返回这个属性。只要把enumerable改成true，Object.values就会返回属性p的值。
```js
...
---
###Object.entries
Object.entries方法返回一个数组，成员是参数对象自身的（不含继承的）所以可遍历（enumerable）属性的键值对数组。
```js
var obj = { foo: 'bar', baz: 42 };
Object.entries(obj)
// [ ["foo", "bar"], ["baz", 42] ]
...
o3.a    // undefined
```
上面代码中，对象o3复制了o2，但是只复制了o2自身的属性，没有赋值它的原型对象o1的属性。
...
z // 3
```
上面代码中，变量x是单纯的解构赋值，所以可以读取对象o继承的属性；变量y和z是双重解构赋值（先是扩展运算符解构赋值，然后是对象的解构赋值），只能读取对象o自身的属性，所以只有变量z可以赋值成功。
...
（2）扩展运算符
扩展运算符（...）用于取出参数对象的所以可遍历属性，拷贝到当前对象之中。
```js
let z = {a: 3, b: 4};
let n = {...z};
n   // {a: 3, b: 4}
...
如果把自定义属性放在扩展运算符后面，就变成了设置新对象的默认属性值。
```js
...
这时，Object.getOwnPropertyDescriptors方法配合Object.defineProperties方法，就可以是实现正确拷贝。
```js
const source = {
  set foo(value) {
    console.log(value);
...
```
ES6规定\__proto__只有浏览器要部署，其他环境不用部署。如果去除\__proto__，上面代码就要改成下面这样。
```js
...
```
这样的层层判断非常麻烦，因此现在有一个提案，引入了“Null传导运算符”（null propagation operator）**?.**,简化上面的写法。
```js
const firstName = message?.body?.user?.firstName || 'defalut';
```
